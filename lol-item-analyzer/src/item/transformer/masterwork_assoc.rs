        ...
        ctx.items.iter_mut().for_each(|(id, item)| {
            if item.is_masterwork {
                if let Some(ids) = item.from.as_ref() {
                    ...
                    mythic_to_masterwork_assoc.push((item.masterwork_from.as_ref().unwrap().to_owned(), id.to_owned()));
                }
            }
        });

        // mythic => masterwork association
        mythic_to_masterwork_assoc.iter().for_each(|(mythic_id, masterwork_id)| {
            if let Some(mythic) = ctx.items.get_mut(mythic_id) {
